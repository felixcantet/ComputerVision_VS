...
void InputHandler2::MoveCursor(const cv::Point &dir) {
    POINT p;
    if (GetCursorPos(&p))
    {
        p.x += dir.x ;
...
void InputHandler2::SetCursorPosition(const cv::Point &pos) {
...
InputType InputHandler2::GetInputTypeFromHand(const HandDetection &hd) {
...
void InputHandler2::PerformInputs(InputType t)
{
    std::for_each(inputs.begin(), inputs.end(), [t](const std::shared_ptr<InputBase>& b)
        {
    		if(b != nullptr)
				b->Perform(t);
        });
...
InputHandler2* InputHandler2::instance = new InputHandler2;
